...
def generate_umap(counts: pd.DataFrame, miR_activity_pvals: pd.DataFrame, 
    ...
    sc.tl.pca(enriched_counts, svd_solver='arpack', random_state=1) 
    sc.pp.neighbors(enriched_counts, n_neighbors=10, n_pcs=30, random_state=1)
    sc.tl.umap(enriched_counts, random_state=1)
