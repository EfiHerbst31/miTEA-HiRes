...
class MyPool(mp.Pool):
    """Extending multiprocessing.pool.Pool with starmap method that can work with tqdm."""
...
def get_data_file_for_extension(data_path: str, data_file_extension: str) -> Optional[str]: 
    ...
    if len(files) > 1:
        raise UsageError('There seem to be more than one %s file. consider passing ' 
                         'preprocess=True in order to merge them to a single file.' 
                         '' % data_file_extension)
...
def get_data_file(data_path: str) -> str: 
    ...
    if not data_file:
        raise UsageError('No \'txt\', \'tsv\', \'mtx\' or \'pkl\' files were found in %s. '
                         'Try passing preprocess=True.' % data_path)
    return data_file
    

def load_mti(species: Optional[str] = constants._SPECIES_HOMO_SAPIENS) -> pd.DataFrame:
    ...
    raise UsageError('Type of species not recognized, either %s are supported. '
                     ...
                     'update load_mti function.' % ' or '.join(constants._SUPPORTED_SPECIES)) 
...
def extract_genes_and_cells(features_tsv_file: str, barcodes_tsv_file: str) -> Tuple[list, list]:
...
def switch_10x_to_txt_single_cell(
        matrix_mtx_file: str, features_tsv_file: str, barcodes_tsv_file: str, 
        sample: int = None, save_to_file: bool = False, path_to_save: str = None) -> pd.DataFrame:
    ...
    genes, cells = extract_genes_and_cells(features_tsv_file, barcodes_tsv_file)
...
def switch_10x_to_txt_spatial(
        matrix_mtx_file: str, features_tsv_file: str, barcodes_tsv_file: str, 
        save_to_file: Optional[bool] = False, path_to_save: Optional[str] = None) -> pd.DataFrame:
    ...
    Args:
        ...
        save_to_file: (optional) Save generated data table to file at 'path_to_save', 
            default = False.
        path_to_save: (optional) Path to save the generated data table if 'save_to_file' is True, 
            default = None.
    ...
    genes, spots = extract_genes_and_cells(features_tsv_file, barcodes_tsv_file)
...
def load_visium(data_path: str, filter_spots: Optional[int] = None) -> pd.DataFrame:
    """Loads visium data.
    ...
    counts = switch_10x_to_txt_spatial(matrix_mtx_file, features_tsv_file, barcodes_tsv_file)
    if filter_spots is not None:
        counts = counts.loc[:, (counts.sum(axis=0) >= filter_spots)]
    return counts

def load_coors_csv_configuration_1(spatial_path: str, counts: pd.DataFrame) -> Optional[pd.DataFrame]:
    """Loads spatial coordinates with configuration #1.

    Coordinates are saved in tissue_positions_list.csv file, in 'array_col' and 'array_row' columns.
...
def load_coors_csv_configuration_2(spatial_path: str, counts: pd.DataFrame) -> Optional[pd.DataFrame]:
...
def load_coors_csv_configuration_3(spatial_path: str, counts: pd.DataFrame) -> Optional[pd.DataFrame]:
    """Loads spatial coordinates with configuration #3.

    Coordinates are saved in tissue_positions.csv file, in 'array_col' and 'array_row' columns.
...
def load_coors_txt_configuration_4(spatial_path: str, counts: pd.DataFrame) -> Optional[pd.DataFrame]:
...
def load_coors_json_configuration_5(spatial_path: str, counts: pd.DataFrame) -> Optional[pd.DataFrame]:
    ...
    try:  
        ...
        logging.debug('Failed loading spatial coordinates with configuration #5.')
...
def get_spatial_coors(data_path: str, counts: pd.DataFrame) -> pd.DataFrame:
    ...
    raise TypeError('Problem loading %s coordinates. Check if the supported configurations match '
                    'your coordinates data structure' % constants._DATA_TYPE_SPATIAL)    

def load_merge_txt_tsv_files(
        files: list, csv_read_fn: functools, 
        sample_size: Optional[int] = constants._MAX_COLS) -> pd.DataFrame:
    """Loads, samples and merges txt or tsv files.

    If needed, matrices are sampled during the process, so that maximum 'sample_size' columns are 
    left at the end. Each reads table is sampled with sample_size/num_of_files cells.
    Duplicated genes are removed. Inner merge is done for multiple files.
...
def load_merge_10x_files(
        mtx_files: list, sample_size: Optional[int] = constants._MAX_COLS) -> pd.DataFrame:
...
def load_single_cell_with_preprocessing(
        dataset_name: str, data_path: str, sample_size: Optional[int] = constants._MAX_COLS
        ) -> pd.DataFrame:
    """Preprocessing and loading scRNAseq data.

    Merging all txt/tsv/mtx files found in data_path (handling also zipped files), sampling columns, 
    and droping duplicated cells in terms of gene expression profiles. If multiple file types are 
    found, pkl is preferable as it is assumed to contain already processed data, either by the user 
    or by previous runs of this software.
    Saving processed file under 'data_path' with 'dataset_name' name and .pkl extension. 
    ...
    if pkl_files:
        ...
        csv_read_fn = functools.partial(pd.read_csv, delimiter='\t', index_col=0)
        counts = load_merge_txt_tsv_files(txt_files, csv_read_fn, sample_size)
    elif mtx_files:
        counts = load_merge_10x_files(mtx_files, sample_size)
    elif tsv_files:
        csv_read_fn = functools.partial(pd.read_csv, sep='\t', index_col=0, on_bad_lines='skip')
        counts = load_merge_txt_tsv_files(tsv_files, csv_read_fn, sample_size)
    else:
        raise UsageError(
            'No \'txt\', \'tsv\', \'mtx\' or \'pkl\' files were found in %s. ' % data_path)
...
def load_single_cell_without_preprocessing(data_path: str) -> pd.DataFrame:
    ...
    if col_len > constants._MAX_COLS:
        logging.info('Reads table is too big, having: %i columns, this might take too long '
                     ...
                     'by passing preprocess=True.' % (col_len, constants._MAX_COLS))
    counts = counts.T.drop_duplicates().T
    return counts
...
def compute_mir_activity(
        counts: pd.DataFrame, miR_list: list, mti_data: pd.DataFrame, results_path: str, 
        cpus: int, debug: Optional[bool] = False) -> pd.DataFrame:
...
def compute_stats_per_cell(
        cell: str, ranked: pd.DataFrame, miR_list: list, mti_data: pd.DataFrame, 
        debug: Optional[bool] = False) -> Tuple[str, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for miR in miR_list:
        ...
        if debug and sum(v) == 0: 
            ...
            logging.debug('No targets were found for microRNA: %s. please check that '
                          '\'species\' parameter is correct. The following '
...
def get_figure_list(miR_list: list, miR_figures: str, mir_activity_list: pd.DataFrame) -> list:
    ...
    Args:
        ...
        mir_activity_list: Sorted list of microRNAs according to their potential interest.
...
def sort_activity_spatial(
        miR_activity_pvals: pd.DataFrame, thresh: float, spots: int, results_path: str, 
        dataset_name: str) -> pd.DataFrame:
    """Computes which microRNAs are active in the most amount of spots.

    The 'Activity Score' of each microRNA is the percentage of spots for which it got a score 
        lower than 'thresh'.
    ...
    Args:
        ...
        results_path: Where to save the sorted list of most abundantly active microRNAs.
        dataset_name: Dataset name.
    
    Returns: 
        Sorted list of microRNAs, from the most abundantly active to the least, over all spots. 
    """
    logging.info('Computing which microRNAs are the most abundantly active within all spots.')
...
def produce_spatial_maps(
        miR_list_figures: list, miR_activity_pvals: pd.DataFrame, spatial_coors: pd.DataFrame, 
        results_path: str, dataset_name: str, mir_activity_list: pd.DataFrame) -> None:
    """Produces a spatial activity map per microRNA in the list.
       Produces also a html file with list of microRNAs, sorted by their abundance of activity.
...
def generate_umap(
        counts: pd.DataFrame, miR_activity_pvals: pd.DataFrame, 
        populations: Optional[list] = None) -> sc.AnnData:
    """Computes UMAP based on gene counts, and enriching it with miR activity scores.
    ...
    if populations:
        ...
        enriched_counts.obs['populations'] = pd.Categorical(
            ...
            populations[0], (np.where(categories['cell_id'].str.contains(
...
def sort_activity_single_cell(
        miR_activity_pvals: pd.DataFrame, populations: Optional[list] = None) -> pd.DataFrame:
    ...
    Returns:
        List of microRNA, sorted according to their potential biological interest. 
    """
    ...
    if populations: # 'Comparative' mode
        ...
        mir_activity_list = sort_activity_single_cell_total_mode(miR_activity_pvals)
    return mir_activity_list

def sort_activity_single_cell_total_mode(miR_activity_pvals: pd.DataFrame) -> pd.DataFrame:
...
def sort_activity_single_cell_comparative_mode(
    ...
    """Sorts active microRNAs according to their differential activity between populations.

    Giving priority to differentially active microRNAs which are also overall the most active.

    Args:
        miR_activity_pvals: Activity results per spot per microRNA.
        populations: List of two unique population string identifiers embedded in cell id.
    ...
    for index, row in mir_activity_list.iterrows():
        ...
        mir_activity_list.loc[index,'fdr_corrected'] = min(
            1, mir_activity_list.loc[index,'ranksum_pval'] * mir_amount / (i + 1))
    ...
    mir_activity_list_high_pval = mir_activity_list[pval_mask_very_active].sort_values(
        by=[col_name_pop_2], ascending=False)
    mir_activity_list_mid_pval = (mir_activity_list[pval_mask_active & 
        ~pval_mask_very_active].sort_values(by=[col_name_pop_2], ascending=False))
    mir_activity_list_low_pval = mir_activity_list[~pval_mask_active]
    mir_activity_list = pd.concat(
        [mir_activity_list_high_pval, mir_activity_list_mid_pval, mir_activity_list_low_pval])
    mir_activity_list = mir_activity_list.rename_axis('MicroRNA')
    return mir_activity_list

def plot_single_cell(
        miR_list_figures: list, enriched_counts: sc.AnnData, results_path: str, dataset_name: str, 
    ...
    Args:
        ...
        results_path: Path to save figures.
        dataset_name: Used for plot name.
        mir_activity_list: Sorted list of most potentially interesting microRNAs.
...
def plot_single_cell_total_mode(
        miR_list_figures: list, enriched_counts: sc.AnnData, results_path: str, dataset_name: str, 
    ...
    for miR, row in mir_activity_list.iterrows():
        ...
        if miR in miR_list_figures:
            ...
            sc.pl.umap(
                enriched_counts, color=miR, title='%s activity (-log10)' % miR, show=False, ax=ax)
            plt.figtext(.02, .02, "Produced by miTEA-HiRes")
...
def plot_single_cell_comparative_mode(
        miR_list_figures: list, enriched_counts: sc.AnnData, results_path: str, dataset_name: str, 
        mir_activity_list: pd.DataFrame, miR_activity_pvals: pd.DataFrame, 
    ...
    for miR, row in mir_activity_list.iterrows():
        if miR in miR_list_figures:
            ...
            path_to_umap_plot = os.path.join(results_path_figures, umap_plot_file_name)
            ...
            path_to_hist_plot = os.path.join(results_path_figures, hist_plot_file_name)
            log10_pvals = -np.log10(miR_activity_pvals)
            pop_1_cols = [col for col in log10_pvals.columns if (populations[0] in col)]
            pop_2_cols = [col for col in log10_pvals.columns if (populations[1] in col)]
            pvals_pop_1 =  log10_pvals.loc[miR, pop_1_cols]
